...
module.exports.run = async (client, message, args) => {
	...
	if (dil == "english") {
		...
		if (!db.has(`suggestion_${message.guildID}_${sugid}.followers`)) return message.channel.createMessage(`This suggestion has sent before v1.0 update (in old form), so you can't follow it.`)
	...
	if (dil == "turkish") {
		...
		if (!db.has(`suggestion_${message.guildID}_${sugid}.followers`)) return message.channel.createMessage(`Bu öneri v1.0 güncellemesinden önce gönderilmiş (eski tip), bundan dolayı takip edemezsin.`)
